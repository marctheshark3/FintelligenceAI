...
class OptimizationConfig(BaseModel):
    ...
    optimizer_type: OptimizerType = Field(default=OptimizerType.MIPROV2, description="Type of optimizer to use")
    strategy: OptimizationStrategy = Field(default=OptimizationStrategy.BALANCED, description="Optimization strategy")
    ...
    max_labeled_demos: int = Field(default=5, description="Maximum labeled demonstrations")
    timeout_minutes: int = Field(default=60, description="Optimization timeout in minutes")
    metric_threshold: float = Field(default=0.8, description="Target metric threshold")
    
    # Component-specific settings
    optimize_retrieval: bool = Field(default=True, description="Optimize retrieval components")
    optimize_generation: bool = Field(default=True, description="Optimize generation components")
    optimize_reranking: bool = Field(default=True, description="Optimize reranking components")
    optimize_agents: bool = Field(default=True, description="Optimize agent components")
    
    # Advanced settings
    use_multi_stage: bool = Field(default=True, description="Use multi-stage optimization")
    save_intermediate: bool = Field(default=True, description="Save intermediate results")
    parallel_execution: bool = Field(default=False, description="Use parallel optimization")
    
    # Model configuration
    llm_config: Dict[str, Any] = Field(default_factory=dict, description="Model configuration overrides")
    evaluation_model: Optional[str] = Field(default=None, description="Model for evaluation")
...
class OptimizationResult(BaseModel):
    ...
    # Component results
    component_results: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Per-component results")
    optimized_prompts: Dict[str, str] = Field(default_factory=dict, description="Optimized prompts")
    optimized_parameters: Dict[str, Any] = Field(default_factory=dict, description="Optimized parameters")
...
class DSPyOptimizer:
    ...
    def optimize_module(
        ...
        try:
            self.logger.info(f"Starting optimization {optimization_id} with {self.config.optimizer_type}")
            ...
            self.logger.info(f"Optimization completed. Final score: {final_score:.3f} "
    ...
    def _create_optimizer(self):
        ...
        try:
            if self.config.optimizer_type == OptimizerType.MIPROV2:
                ...
                return dspy.COPRO(
                    metric=self._get_default_metric(),
                    breadth=self.config.num_trials
                )
            else:
                # Default to MIPROv2
                return dspy.MIPROv2(
            ...
            self.logger.warning(f"Failed to create {self.config.optimizer_type}, using basic optimizer: {e}")
