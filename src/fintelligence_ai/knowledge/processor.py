...
class DocumentProcessor:
    ...
    def process_documents(
    ...
    ) -> tuple[list[DocumentChunk], ProcessingStats]:
        ...
        stats = ProcessingStats(
            ...
            average_chunk_size=sum(len(chunk.content) for chunk in all_chunks)
            // len(all_chunks)
            if all_chunks
            else 0,
    ...
    def _chunk_section(
        self, document: Document, content: str, section_type: str, start_index: int
    ) -> list[DocumentChunk]:
    ...
    def _create_chunk(
        self, document: Document, content: str, chunk_type: str, chunk_index: int
    ) -> DocumentChunk:
...
class ErgoScriptProcessor(DocumentProcessor):
    ...
    def _chunk_code_section(
        self, document: Document, content: str, start_index: int
    ) -> list[DocumentChunk]:
    ...
    def _analyze_code_content(self, code: str) -> dict:
        ...
        metadata["has_contract"] = any(
            keyword in code for keyword in ["sigmaProp", "proveDlog", "SELF"]
        )
        metadata["has_conditional"] = any(
            keyword in code for keyword in ["if", "else", "match"]
        )
        metadata["has_crypto"] = any(
            keyword in code for keyword in ["hash", "signature", "proveDlog"]
        )
        metadata["has_tokens"] = any(
            keyword in code for keyword in ["TOKENS", "token", "asset"]
        )
