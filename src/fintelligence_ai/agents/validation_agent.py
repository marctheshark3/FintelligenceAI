...
class SemanticAnalysis(dspy.Signature):
    ...
    is_semantically_correct: bool = dspy.OutputField(
        desc="Whether semantics are correct"
    )
...
class ValidationAgent(BaseAgent):
    ...
    async def _handle_syntax_validation(
        ...
        try:
            ...
            result = validator(code=code, context="ErgoScript syntax validation")
    ...
    async def _handle_semantic_analysis(
        ...
        try:
            ...
            result = analyzer(code=code, use_case=use_case)
        ...
        try:
            ...
            if criteria.syntax_check:
                validation_tasks.append(
                    self._handle_syntax_validation(code, context, metadata)
                )

            if criteria.semantic_check:
                validation_tasks.append(
                    self._handle_semantic_analysis(code, context, metadata)
                )

            if criteria.security_check:
                validation_tasks.append(
                    self._handle_security_analysis(code, context, metadata)
                )
            ...
            if (
                ...
                and len(results) > 0
                and not isinstance(results[0], Exception)
            ):
    ...
    async def _handle_security_analysis(
        ...
        try:
            ...
            result = analyzer(code=code, context=security_context)
    ...
    def _build_validation_context(
        ...
        if context and context.context_data:
            context_parts.append(
                f"Domain: {context.context_data.get('domain', 'general')}"
            )
            context_parts.append(
                f"Use case: {context.context_data.get('use_case', 'general')}"
            )
