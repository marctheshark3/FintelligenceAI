...
class AgentOrchestrator(BaseAgent):
    ...
    async def _execute_task_impl(
        ...
        if task_type == TaskType.CODE_GENERATION:
            ...
            return await self._delegate_to_agent(
                "validation", task_type, content, context, metadata
            )
        else:
    ...
    async def _research_phase(
        ...
        try:
            ...
            if research_task_result.success:
                ...
                self.logger.warning(
                    f"Research phase failed: {research_task_result.error_message}"
                )
                return {
                    "findings": "No research data available",
                    "sources": "",
                    "recommendations": "",
                }
    ...
    async def _generation_phase(
        ...
        if generation_task_result.success:
            ...
            raise Exception(
                f"Generation phase failed: {generation_task_result.error_message}"
            )
    ...
    async def _validation_phase(
        ...
        try:
            ...
            if validation_task_result.success:
                ...
                self.logger.warning(
                    f"Validation phase failed: {validation_task_result.error_message}"
                )
    ...
    def _enhance_context_with_research(
        ...
        if original_context is None:
            original_context = ConversationContext(
                session_id=uuid4(), conversation_history=[], context_data={}
            )
    ...
    async def _delegate_to_agent(
        ...
        task_result = await agent.execute_task(
            task_type=task_type, content=content, context=context, metadata=metadata
        )
    ...
    async def get_agent_status(self) -> dict[str, Any]:
        ...
        status = {"orchestrator": self.get_status(), "agents": {}}
