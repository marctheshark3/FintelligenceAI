...
class AgentMessage(BaseModel):
    ...
    recipient_id: Optional[str] = Field(
        default=None, description="ID of the receiving agent"
    )
    task_type: Optional[TaskType] = Field(
        default=None, description="Type of task being requested"
    )
    ...
    correlation_id: Optional[UUID] = Field(
        default=None, description="ID for tracking related messages"
    )
...
class TaskResult(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Task completion timestamp"
    )
...
class AgentCapability(BaseModel):
    ...
    supported_task_types: list[TaskType] = Field(
        description="Task types this capability supports"
    )
    required_tools: list[str] = Field(
        default_factory=list, description="Required tools for this capability"
    )
...
class AgentConfig(BaseModel):
    ...
    llm_config: dict[str, Any] = Field(
        default_factory=dict, description="LLM model configuration"
    )
    tools_config: dict[str, Any] = Field(
        default_factory=dict, description="Tools configuration"
    )
...
class ConversationContext(BaseModel):
    ...
    current_task: Optional[TaskType] = Field(
...
class ValidationCriteria(BaseModel):
    ...
    optimization_check: bool = Field(
        default=False, description="Check for optimization opportunities"
    )
    ...
    test_scenarios: list[str] = Field(
        default_factory=list, description="Test scenarios to validate"
    )
...
class ErgoScriptRequest(BaseModel):
    ...
    use_case: Optional[str] = Field(
        default=None, description="Specific use case (token, auction, etc.)"
    )
    ...
    validation_criteria: ValidationCriteria = Field(
        default_factory=ValidationCriteria, description="Validation criteria"
    )
...
class ErgoScriptResponse(BaseModel):
    ...
    gas_estimate: Optional[float] = Field(
        default=None, description="Estimated gas cost"
    )
    ...
    example_usage: Optional[str] = Field(
        default=None, description="Example usage of the script"
    )
    metadata: dict[str, Any] = Field(
