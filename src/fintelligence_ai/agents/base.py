...
class BaseAgent(ABC):
    ...
    async def process_message(self, message: AgentMessage) -> Optional[AgentMessage]:
        ...
        try:
            ...
            return self._create_error_message(
                original_message=message, error_message=str(e)
            )
    ...
    async def execute_task(
        ...
        try:
            if not self.can_handle_task(task_type):
                raise ValueError(
                    f"Agent {self.agent_id} cannot handle task type: {task_type}"
                )

            # Execute the specific task implementation
            result = await self._execute_task_impl(
                task_type, content, context, metadata
            )
    ...
    async def _handle_notification(
        ...
        self.logger.info(
            f"Received notification from {message.sender_id}: {message.content}"
        )
        return None
