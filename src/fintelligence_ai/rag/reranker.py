...
from .models import Query, RetrievalResult, RetrievalConfig
...
class DocumentReranker:
    ...
    def _compute_cross_encoder_scores(self, pairs: List[Tuple[str, str]]) -> List[float]:
    ...
    def _apply_domain_scoring(
        ...
        for result in results:
            ...
            if any(keyword in query_lower for keyword in scoring_factors["code_keywords"]):
            ...
            if any(keyword in query_lower for keyword in scoring_factors["security_keywords"]):
                if "security" in result.metadata.tags:
                    adjustment += 0.15
            
            # Boost for performance-related queries
            if any(keyword in query_lower for keyword in scoring_factors["performance_keywords"]):
                if result.metadata.performance_notes:
                    adjustment += 0.1
            
            # Complexity matching
            for complexity, keywords in scoring_factors["complexity_preferences"].items():
