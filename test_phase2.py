...
async def test_optimization_framework():
    ...
    try:
        ...
        from fintelligence_ai.core import DSPyOptimizer, OptimizationConfig
...
async def test_evaluation_framework():
    ...
    try:
        ...
        ergoscript_metrics = ergoscript_evaluator.evaluate_generated_script(
            test_code, requirements=["token creation", "output validation"]
        )

        print(
            f"✅ ErgoScript evaluation: {ergoscript_metrics.syntax_correctness:.2f} syntax score"
        )
        ...
        rag_metrics = rag_evaluator.evaluate_retrieval_performance(
            "How to create tokens", test_documents
        )

        print(
            f"✅ RAG evaluation: {rag_metrics.retrieval_relevance:.2f} relevance score"
        )
        ...
        agent_metrics = agent_evaluator.evaluate_agent_performance(
            "Create a token contract", mock_agent_response
        )

        print(
            f"✅ Agent evaluation: {agent_metrics.task_completion_rate:.2f} completion rate"
        )
...
async def test_api_endpoints():
    ...
    try:
        ...
        optimization_routes = [
            path for path in route_paths if path.startswith("/optimization")
        ]
        ...
        available_endpoints = [
            ep for ep in key_endpoints if any(ep in path for path in route_paths)
        ]

        print(
            f"✅ Key endpoints available: {len(available_endpoints)}/{len(key_endpoints)}"
        )
